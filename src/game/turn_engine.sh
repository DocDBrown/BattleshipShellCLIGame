...
te__parse_coord_to_zero_based() {
	...
	validate_coordinate "${coord}" "${size_current}" || return 3
...
te_human_shoot() {
	...
	if [[ "${cur_state}" == "hit" || "${cur_state}" == "miss" ]]; then
		if [ -n "${TE__ON_SHOT_RESULT_CB}" ] && te__require_fn "${TE__ON_SHOT_RESULT_CB}" >/dev/null 2>&1; then
			"${TE__ON_SHOT_RESULT_CB}" "human" "${coord}" "already_shot" "" "" "${TE__STATS_SHOTS}" "${TE__STATS_HITS}" "${TE__STATS_MISSES}" || true
		fi
		return 0
	fi
	...
	if [ -n "${owner}" ]; then
		...
		if [ -n "${TE__ON_SHOT_RESULT_CB}" ] && te__require_fn "${TE__ON_SHOT_RESULT_CB}" >/dev/null 2>&1; then
			if [[ "${sunk}" == "true" ]]; then
				"${TE__ON_SHOT_RESULT_CB}" "human" "${coord}" "sunk" "${owner}" "${ship_name}" "${TE__STATS_SHOTS}" "${TE__STATS_HITS}" "${TE__STATS_MISSES}" "${remaining}" || true
			else
				"${TE__ON_SHOT_RESULT_CB}" "human" "${coord}" "hit" "${owner}" "${ship_name}" "${TE__STATS_SHOTS}" "${TE__STATS_HITS}" "${TE__STATS_MISSES}" "${remaining}" || true
			fi
		fi
		local win="false"
		...
		if [[ "${win}" == "true" ]]; then
			if [ -n "${TE__ON_SHOT_RESULT_CB}" ] && te__require_fn "${TE__ON_SHOT_RESULT_CB}" >/dev/null 2>&1; then
				"${TE__ON_SHOT_RESULT_CB}" "human" "${coord}" "win" "${owner}" "${ship_name}" "${TE__STATS_SHOTS}" "${TE__STATS_HITS}" "${TE__STATS_MISSES}" "0" || true
			fi
		fi
		return 0
	else
		...
		if [ -n "${TE__ON_SHOT_RESULT_CB}" ] && te__require_fn "${TE__ON_SHOT_RESULT_CB}" >/dev/null 2>&1; then
			"${TE__ON_SHOT_RESULT_CB}" "human" "${coord}" "miss" "" "" "${TE__STATS_SHOTS}" "${TE__STATS_HITS}" "${TE__STATS_MISSES}" || true
		fi
		return 0
	fi
}
