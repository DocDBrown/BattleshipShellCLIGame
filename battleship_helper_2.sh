...
# Public testable entry: minimal game_flow-like CLI handler for a few flags.
# Purpose: provide a tiny, well-scoped surface for tests to assert behavior for --manual.
# This function intentionally implements only the small portion of the original game_flow
# that is required by tests: handling --manual and returning the documented exit code 3.
game_flow_entry() {
	# Accepts the same args as a CLI; returns 3 for --manual with an explanatory message.
	if [ "${#}" -eq 0 ]; then
		return 0
	fi
	while [ "$#" -gt 0 ]; do
		...
		--help | -h)
			...
			;;
		*)
			# Ignore unknown args in this lightweight test shim
			shift
			;;
		esac
	done
